...
class IntersectionEditor:
    ...
    def _preset_webster(self) -> None:
        ...
        # Cycle change via the slider so rescaling is consistent
        # But webster changes cycle length too — we need to update the plan's cycle
        # which is derived from sum(total_time). Since total_time = green + Y + AR,
        # updating greens automatically updates cycle via property.
        self._resync_sliders()
    ...
    def _resync_sliders(self) -> None:
        ...
        if self._offset_slider is not None:
            ...
            if self._offset_slider.value > pp.cycle_length:
